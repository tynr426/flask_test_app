 ...
 * 自定义的LinearLayout 也继承自ViewGroup 并且重写抽象类ViewGrouop的几个方法:onMeasure(),
 * onLayout(),dispathDraw()  三个方法的意思分别是:第一个onMeasure()是用来计算控件以及子控件
 * 所占用的区域,第二个onLayout()是控制子控件的换行
 */
public class FixGridLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        if(mCellWidth<1||mCellHeight<1)return;
